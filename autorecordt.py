...
def setup_adc(adc, again,dgain,pregain):

	#again and dgain are lists with 2 elements, one for each channel
    ...
    adc.set_analog_gain(1, analog_gain_db=again[0])
    adc.set_analog_gain(2, analog_gain_db=again[1])
    ...
    adc.set_digital_gain(channel=1, digital_gain_db = dgain[0], muted=False, soft_step=True, ganged=False)
    adc.set_digital_gain(channel=2, digital_gain_db = dgain[1], muted=False, soft_step=True, ganged=False)
    
    adc.set_pre_input_gain(channel=1,pre_input_gain_db = pregain[0])
    adc.set_pre_input_gain(channel=2,pre_input_gain_db = pregain[1])
...
if config.sections() == []:
    ...
    #debug
    setup_adc(adc1,again=again,dgain=dgain,pregain=pregain)
